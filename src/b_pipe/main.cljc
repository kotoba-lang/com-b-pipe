(ns b_pipe.main
  "Kotodama WASM entrypoint for the B_pipe clean-room actor (L4) — Clojure port.
  ...
  facts (`b_pipe.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "b_pipe")
(def tier "L4")
...
(def entity-specs
  [{:entity "Instrument" :plural "instruments" :id-prefix "bpipe_ins"
   ...
   {:entity "Position" :plural "positions" :id-prefix "bpipe_pos"
...
(defn emit-facts
  "EAVT facts for one record: {\"b_pipe.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "b_pipe-compat" :tier tier :entities entities} 200])
...
(defn start! [] :b_pipe-compat/ready)
